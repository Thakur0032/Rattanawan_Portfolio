        ...
        >
          {skillCategories.map((category, idx) => (
            <motion.div 
              ...
              className="glass p-8 rounded-3xl border-accent relative group hover:border-accent transition-colors"
            >
              ...
              <div className="flex flex-wrap gap-2">
                {category.skills.map((skill, sIdx) => (
                  <span 
                    key={sIdx}
                    className="px-3 py-1.5 text-sm font-medium bg-secondary border border-accent rounded-lg text-muted-foreground group-hover:text-[#020C1B]/90 hover:bg-accent transition-all cursor-default"
                  >
                    {skill}
                  </span>
                ))}
